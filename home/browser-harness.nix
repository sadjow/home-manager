...
in
{
  ...
  home.activation.browserHarness = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    browser_harness_recording_status="$(
      BH_TELEMETRY=0 "$browser_harness_bin" recordings 2>/dev/null || true
    )"
    if printf '%s\n' "$browser_harness_recording_status" \
      | ${pkgs.gnugrep}/bin/grep -Fq '(default)'; then
      BH_TELEMETRY=0 \
        "$browser_harness_bin" recordings disable >/dev/null
    fi
  '';
}
